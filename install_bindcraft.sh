...
$pkg_manager install \
  ...
  jupyter jupyterlab ffmpeg fsspec py3dmol \
...
############################################################################################################
################## Install FreeSASA (optional, improves SASA calcs)
echo "Installing FreeSASA"
python -m pip install freesasa || echo "Warning: freesasa install failed (fallback to Biopython SASA)"

############################################################################################################
################## Fix permissions for executables
chmod +x "${install_dir}/functions/dssp" || { echo "chmod dssp failed"; exit 1; }
chmod +x "${install_dir}/functions/sc"   || echo "Warning: sc binary missing"
chmod +x "${install_dir}/functions/FASPR" || echo "Warning: FASPR binary missing"

# Skip DAlphaBall.gcc since it's only for PyRosetta
echo "Skipping DAlphaBall.gcc setup (no PyRosetta build)"
...
################## Finish
t=$SECONDS 
echo -e "\nSuccessfully finished BindCraft (no PyRosetta) installation!"
